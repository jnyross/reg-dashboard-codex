...
describe("crawlSources", () => {
    ...
    it("parses RSS feed items and includes provenance links", async () => {
      ...
      <item>
        ...
        const result = await (0, crawler_1.crawlSources)([source]);
    ...
    it("deduplicates duplicate feed items from same source", async () => {
      ...
      <item>
        ...
        const result = await (0, crawler_1.crawlSources)([source]);
        expect(result.sourceResults[0]).toEqual(expect.objectContaining({
            sourceId: source.id,
            itemCount: 1,
            error: null,
        }));
    ...
    it("parses webpage content and recovers from bad sources", async () => {
        ...
        const result = await (0, crawler_1.crawlSources)([goodSource, badSource]);
